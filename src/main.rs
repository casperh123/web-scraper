...
async fn main() {
    ...
    let (crawled_tx, crawled_rx) = mpsc::unbounded_channel::<Url>();
...
async fn save_websites(db: DbConn, mut crawled: UnboundedReceiver<Url>) {
    loop {
        let crawled_domain = crawled.recv().await.expect("Error while getting domain from database receiver");
        add_website(&db, crawled_domain.to_string()).await;
    }
}
